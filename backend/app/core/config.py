...
class Settings(BaseSettings):
    ...
    def DATABASE_URL(self) -> str:
        """Build async SQLite DSN from individual components."""
        return "sqlite+aiosqlite:///./data/fberaucracy.db"

    # ── OpenRouter AI ─────────────────────────────────────────────────────────
    OPENROUTER_API_KEY: str = Field(default="")
    OPENROUTER_BASE_URL: str = Field(default="https://openrouter.ai/api/v1")
    OPENROUTER_MODEL: str = Field(default="google/gemini-2.5-flash")
    OPENROUTER_APP_TITLE: str = Field(default="f-idarty")
    OPENROUTER_SITE_URL: Optional[str] = Field(default=None)

    # Note: Embeddings are handled locally by ChromaDB's built-in model
    # (all-MiniLM-L6-v2). No external embedding API key is needed.
