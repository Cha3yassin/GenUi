...
# ChromaDB ships with a default embedding function (all-MiniLM-L6-v2) that
# runs locally — no external API key needed.  This is used for both ingestion
# and query-time embedding so vectors are always compatible.
from chromadb.utils import embedding_functions as _ef

_default_ef = _ef.DefaultEmbeddingFunction()  # all-MiniLM-L6-v2, 384-dim


async def embed_text(text: str) -> List[float]:
    """
    Generate a dense vector embedding for the given text using ChromaDB's
    built-in sentence-transformer model (runs locally, no API key required).

    Runs the blocking model call in a thread pool to stay non-blocking.
    """
    loop = asyncio.get_event_loop()
    # DefaultEmbeddingFunction expects a list and returns a list of embeddings
    result = await loop.run_in_executor(
        None,
        lambda: _default_ef([text]),
    )
    return result[0]
