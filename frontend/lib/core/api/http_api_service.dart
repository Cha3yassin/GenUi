...
class HttpApiService implements ApiService {
  ...
  Future<QuizWidgetModel> generateQuizWidget({
    ...
    try {
      final uri = Uri.parse('${AppConfig.apiV1}/gen-ui/quiz');
      final body = <String, dynamic>{
        'topic': topic,
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (data['type'] == 'error') {
          throw ApiException(data['message'] ?? 'Erreur IA', data['code']);
        }
        return QuizWidgetModel.fromJson(data);
      }
      ...
      throw ApiException(
          'Le service IA met trop de temps a generer le quiz.', 'TIMEOUT');
    } on SocketException {
      throw ApiException(
          'Impossible de connecter au serveur IA.', 'NETWORK_ERROR');
    }
