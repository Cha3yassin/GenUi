...
class MockApiService implements ApiService {
  ...
  Future<List<ProcedureSummaryModel>> searchProcedures(String query,
    ...
    final procedures =
        _procedureSummariesJson.map(ProcedureSummaryModel.fromJson).toList();
  ...
  @override
  Future<QuizWidgetModel> generateQuizWidget({
    required String topic,
    String? role,
    String? language,
  }) async {
    await Future<void>.delayed(_networkDelay);
    return QuizWidgetModel.fromJson({
      'type': 'quiz_widget',
      'title': 'AI generated quiz',
      'subtitle': 'Generated widget plan from the mock AI service.',
      'topic': topic,
      'language': language ?? 'en',
      'questions': [
        {
          'prompt': 'What should you verify before an office visit?',
          'options': [
            {'text': 'Documents, fees, and office hours'},
            {'text': 'Only the street name'},
            {'text': 'Nothing'},
          ],
          'correct_index': 0,
          'explanation': 'A prepared checklist saves repeat visits.',
        },
        {
          'prompt': 'Why keep copies with original documents?',
          'options': [
            {'text': 'Copies replace originals'},
            {'text': 'Offices may keep copies after checking originals'},
            {'text': 'They are never useful'},
          ],
          'correct_index': 1,
          'explanation': 'Many administrative files require a retained copy.',
        },
        {
          'prompt': 'What should you do if requirements are unclear?',
          'options': [
            {'text': 'Guess'},
            {'text': 'Skip the process'},
            {'text': 'Ask or use a guide before travelling'},
          ],
          'correct_index': 2,
          'explanation': 'Checking first reduces wasted trips.',
        },
      ],
      'result_bands': [
        {
          ...
          'message': 'The AI widget result is rendered from JSON blocks.',
          'blocks': [
            {
              'type': 'info_card',
              'data': {
                'title': 'AI block',
                'body': '- This checklist came from the quiz widget JSON.',
                'icon': 'shield',
              },
            },
          ],
  ...
  Future<void> saveToHistory(
      String token, String userMessage, Map<String, dynamic> aiResponse) async {
    // No-op in mock
  }
}
