...
final quizGenerationProvider = StateProvider<int>((ref) => 0);
...
final generatedQuizProvider = FutureProvider<QuizWidgetModel>((ref) {
  final locale = ref.watch(localeProvider);
  final role = ref.watch(userRoleProvider);
  final generation = ref.watch(quizGenerationProvider);
  final baseTopic = locale == 'en'
      ? 'Prepare for a Tunisian administrative office visit'
      : 'Se preparer pour une visite administrative en Tunisie';
  return ref.watch(apiServiceProvider).generateQuizWidget(
        topic:
            '$baseTopic. Fresh generation variant ${generation + 1}; change the questions and vary correct answer positions.',
        role: role,
        language: locale,
      );
});
