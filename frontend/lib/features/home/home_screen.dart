    ...
    return Column(
      ...
      children: [
        ...
        AsyncValueWidget<List<CategoryModel>>(
          ...
          data: (categories) => _CategoryGrid(categories: categories, locale: locale),
        ),
    ...
    return Column(
      ...
      children: [
        ...
        _stagger(i++, _ActionPriorityCard(
          ...
          items: [
            (AppStrings.get('sub_sarl', l), 'register-company-sarl-tunisia'),
        ...
        _stagger(i++, _ActionPriorityCard(
          ...
          items: [
            ...
            (AppStrings.get('sub_declaration', l), 'tax-declaration-tunisia'),
...
class _EnterpriseFilterChips extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final filters = [
      (AppStrings.get('filter_fiscalite', locale), Icons.account_balance_rounded, AppTheme.actionYellow),
      (AppStrings.get('filter_creation', locale), Icons.add_business_rounded, AppTheme.actionOrange),
      (AppStrings.get('filter_social', locale), Icons.people_rounded, AppTheme.actionGreen),
      (AppStrings.get('filter_registre', locale), Icons.folder_rounded, AppTheme.actionBlue),
    ];
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(f.$1, style: GoogleFonts.plusJakartaSans(
                    fontSize: 13, fontWeight: FontWeight.w600, color: f.$3,
                  )),
                ],
              ),
            ),
...
class _ActionPriorityCard extends StatelessWidget {
  const _ActionPriorityCard({
    ...
    required this.accentColor, required this.items, required this.consultLabel,
  });
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(subtitle, style: GoogleFonts.plusJakartaSans(
                                ...
                                color: AppTheme.mutedInk, height: 1.3,
                              )),
                            ],
                          ),
                        ),
                    ...
                    ...items.map((item) => _SubItem(
                      label: item.$1, slug: item.$2, color: accentColor,
                    )),
                    // Consult link
                    const SizedBox(height: 8),
                    Text(consultLabel, style: GoogleFonts.plusJakartaSans(
...
class _SubItem extends StatelessWidget {
  const _SubItem({required this.label, required this.slug, required this.color});
  final String label;
  final String slug;
  final Color color;
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Icon(Icons.north_east_rounded, size: 16, color: AppTheme.mutedInk.withOpacity(0.4)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Wrap(spacing: 10, runSpacing: 10, children: _individualChips(context, locale)),
      ],
    );
  }

  List<Widget> _individualChips(BuildContext context, String locale) => [
    SahilChipButton(label: AppStrings.get('buy_car', locale), icon: Icons.directions_car_rounded,
      onPressed: () => context.push(RoutePaths.procedure('buy-used-car'))),
    SahilChipButton(label: AppStrings.get('passport_renewal', locale), icon: Icons.badge_rounded,
      onPressed: () => context.push(RoutePaths.procedure('passport-renewal'))),
    SahilChipButton(label: AppStrings.get('marriage_certificate', locale), icon: Icons.favorite_rounded,
      onPressed: () => context.push(RoutePaths.procedure('marriage-certificate'))),
    SahilChipButton(label: AppStrings.get('national_id_card', locale), icon: Icons.credit_card_rounded,
      onPressed: () => context.push(RoutePaths.procedure('national-id-card-cin'))),
  ];
}
    ...
    return Row(
      children: [
        Builder(builder: (ctx) => Container(
          width: 42, height: 42,
          decoration: BoxDecoration(
            color: AppTheme.paper, borderRadius: BorderRadius.circular(14),
            border: Border.all(color: AppTheme.borderLight),
          ),
          child: IconButton(icon: const Icon(Icons.menu_rounded, size: 20),
            onPressed: () => Scaffold.of(ctx).openDrawer(), padding: EdgeInsets.zero),
        )),
        const Spacer(),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: Text(
        ...
        style: GoogleFonts.plusJakartaSans(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.terracotta),
      )),
    );
  }
}
    ...
    return Column(
      ...
      children: [
        Text(AppStrings.get('app_name', locale),
          style: GoogleFonts.playfairDisplay(fontSize: 34, fontWeight: FontWeight.w800,
            color: AppTheme.ink, letterSpacing: -0.6)),
        const SizedBox(height: 6),
        Text(AppStrings.get('app_tagline', locale),
          style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: AppTheme.mutedInk, height: 1.5)),
      ],
    );
  }
}
...
class _LanguageToggle extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppTheme.paper, borderRadius: BorderRadius.circular(12),
        border: Border.all(color: AppTheme.borderLight)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        _LangChip(label: 'AR', isActive: locale == 'ar',
          onTap: () => ref.read(localeProvider.notifier).setLocale('ar')),
        _LangChip(label: 'EN', isActive: locale == 'en',
          onTap: () => ref.read(localeProvider.notifier).setLocale('en')),
        _LangChip(label: 'FR', isActive: locale == 'fr',
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return CategoryCard(category: cat, locale: locale,
...
class _TrustCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppTheme.paper, borderRadius: BorderRadius.circular(16),
        border: Border.all(color: AppTheme.borderLight)),
      child: Row(children: [
        Container(width: 48, height: 48,
          decoration: BoxDecoration(color: AppTheme.olive.withOpacity(0.1), borderRadius: BorderRadius.circular(16)),
          child: const Icon(Icons.fact_check_rounded, color: AppTheme.olive, size: 22)),
        const SizedBox(width: 16),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(AppStrings.get('trust_title', locale), style: Theme.of(context).textTheme.titleSmall),
