    ...
    return Column(
      children: [
        for (int i = 0; i < widget.results.length; i++) ...[
          if (widget.results[i].isAdministrative)
            ...
            _cascadeItem(i, totalMs, const _NonAdministrativeWarning()),
          const SizedBox(height: 12),
        ],
      ],
    );
...
class _NonAdministrativeWarning extends ConsumerWidget {
  const _NonAdministrativeWarning();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    return Card(
      color: AppTheme.terracotta.withOpacity(0.05),
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(16),
        side: BorderSide(color: AppTheme.terracotta.withOpacity(0.2)),
      ),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Ce terme ne correspond à aucune démarche administrative tunisienne. Essayez: passeport, carte grise, SARL, permis de conduire...',
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                const _SuggestionChipSmall(label: 'passeport'),
                const _SuggestionChipSmall(label: 'carte grise'),
                const _SuggestionChipSmall(label: 'SARL'),
              ],
            ),
          ],
        ),
...
class _SuggestionChipSmall extends ConsumerWidget {
  const _SuggestionChipSmall({required this.label});
  final String label;
